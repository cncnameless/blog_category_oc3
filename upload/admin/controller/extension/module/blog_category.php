...
class ControllerExtensionModuleBlogCategory extends Controller {
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/blog_category', 'user_token=' . $this->session->data['user_token'])
        );

        $data['action'] = $this->url->link('extension/module/blog_category', 'user_token=' . $this->session->data['user_token']);

        $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module');

        if (isset($this->request->post['module_blog_category_status'])) {
            $data['status'] = $this->request->post['module_blog_category_status'];
        } else {
            $data['status'] = $this->config->get('module_blog_category_status');
        }
